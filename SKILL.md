...
- each numbered item must contain exactly two physical lines in this order: User then LLM
- use the XML-labelled contract below to understand the required layout, but do not output the XML tags

<required_layout>
<line_1>Final title only</line_1>
<line_2_onward>
1. User: ...
   LLM: ...
2. User: ...
...
- markdown layout requirements (to avoid extra blank lines and list drift):
  - do not use unordered list markers such as `-` or `*` for User/LLM lines
  - keep `User:` on the same physical line as the ordered-list marker
  - indent each `LLM:` line with exactly three spaces
  - do not insert any blank lines inside the ordered list
  - do not wrap a numbered item into extra paragraphs; if a summary is too long, shorten it instead
- each numbered item must summarize only that turn pair
- keep each User/LLM line concise but specific
...
- if a user turn changes direction, make that change explicit in the User line
...
Do not output `<required_output_example>` or any other XML tags in the final answer.
